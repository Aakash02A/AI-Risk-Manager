...
import { Sliders, Plus, Search, ChevronRight, Bell } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const moduleTitles: Record<ModuleType, { title: string; subtitle: string }> = {
    operations: {
      ...
      subtitle: 'Live chargeback disputes, probability routing, and evidence response engine',
    },
    risk: {
      ...
      <div className="px-6 py-3.5 flex items-center justify-between">
        ...
        <div>
          <div className="flex items-center space-x-1.5 text-xs text-slate-500 mb-0.5">
            ...
            <span className="capitalize font-medium text-emerald-700">{activeModule}</span>
          </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="relative hidden md:block w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 bg-slate-100 border border-slate-200 rounded-lg text-xs focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
            />
          </div>
